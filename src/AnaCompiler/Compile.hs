...
import AnaCompiler.Asm (Arg (Const, Label, Reg, RegOffset), Instruction (..), Reg (R10, RAX, RBX, RCX, RDI, RSP), toAsm)
...
calcDefTyp defEnv (DFun _ args ret body) =
  let lastBodyExpr = last body
      lastBodyExprTyp =
        calcTyp lastBodyExpr args defEnv
  ...
  case expr of
    ...
    ETail name ->
      case lookup name typEnv of
        ...
        Just _ -> pure $ throw $ AnaCompilerException [printf "Type mismatch: unknown pair %s type" name]
    EVector exprs ->
      let exprsTypList = foldl (\acc e -> calcTyp e typEnv defTypEnv : acc) [] exprs
    ...
    EVecSet vec _ exprVecSet ->
      case lookup vec typEnv of
        Nothing -> throw $ AnaCompilerException [printf "variable identifier %s unbound" vec]
        Just vecTyp -> do
          TypValidated newTyp <- calcTyp exprVecSet typEnv defTypEnv
          if vecTyp == newTyp
    ...
    EApp name listParams ->
      case lookup name defTypEnv of
        Just (typ, args) ->
          let paramsCheck =
                sequence (reverse (foldl (\acc e -> calcTyp e typEnv defTypEnv : acc) [] listParams))
                  >>= ( \paramsTyps ->
      ...
      let a = case lookup vec s of
            ...
            Just i ->
              [ IMov (Reg RAX) (stackloc i),
                ...
                IJge overflowIndex,
                IMov (Reg RAX) (RegOffset (8 * (item + 1)) RAX),
                IJmp endLabel,
